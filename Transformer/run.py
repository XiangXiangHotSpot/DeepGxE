...
def get_hyper_parameter():
    ...
    parser.add_argument("--batch_size", type=int, default=256)                   # mini-batch大小
    ...
    parser.add_argument("--pheno_normalize", type=int, default=1)                   # 是否对表型数据做归一化
    ...
    parser.add_argument("--nhead", type=int, default=3)                   #S1 S2 S3
    parser.add_argument("--num_layers", type=int, default=3)              #S1 S2 S3
    parser.add_argument("--mlp_ratio", type=int, default=16)              #S1 S2 S3
...
def k_fold_cross_validation(config):
    ...
    for fold in range(config.fold):
        ...
        # torch.save(model.state_dict(), "model_pth/fold-model-{}.pth".format(current_fold))
...
def evaluation(
    ...
    for test_iter, val_batch_data in enumerate(val_loader):
        ...
        if config.pheno_normalize == 1:
            ...
            # phenotype = phenotype * (max_pheno_value - min_pheno_value) + min_pheno_value
...
def train(
    ...
    # training start
    for epoch_index in tqdm(range(config.epoch)):  # epoch 循环
        print(
            "A Transformer Model in Scenario %s -——--Fold %s -- Epoch %s Is Training---------" % (
        ...
        for train_iter, train_batch_data in enumerate(train_loader):
            # 网络中有 BatchNorm 和 Dropout 必须写， 下 .eval() 同     
            model.train()
